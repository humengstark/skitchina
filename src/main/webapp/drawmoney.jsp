...
<%@ page import="com.skitchina.model.DrawMoney" %>
...
<%
    List<DrawMoney> drawMoneys = (List<DrawMoney>) session.getAttribute("drawMoneys");
    int pagesNow = (Integer) session.getAttribute("pagesNow");
    int drawMoneysNum = (Integer) session.getAttribute("drawMoneysNum");
    double pagesAll1 = (double) drawMoneysNum / 10;
    int pagesAll;
    if (pagesAll1 > drawMoneysNum / 10) {
        pagesAll = drawMoneysNum / 10 + 1;
    } else {
        pagesAll = drawMoneysNum / 10;
    }
%>
<!--_meta 作为公共模版分离出去-->
<HTML>
    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            ...
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l"></span> <span class="r">共有数据：<strong><%=drawMoneysNum%></strong> 条</span> </div>
            <div class="mt-10">
                <table class="table table-border table-bordered table-bg table-sort" style="table-layout: fixed">
                    ...
                    <%for (DrawMoney drawMoney:drawMoneys){%>
                    <tr class="drawMoneys">
                        <td><%=drawMoney.getId()%>
                        </td>
                        <td><%=drawMoney.getBank()%>
                        </td>
                        <td><%=drawMoney.getBankcard()%>
                        </td>
                        <td><%=drawMoney.getRealname()%>
                        </td>
                        <td><%=drawMoney.getDraw_time().substring(0,19)%>
                        </td>
                        <td><%=drawMoney.getMoney()%>
                        </td>
                        <%if (drawMoney.getDraw_condition()==0){%>
                        <td>
                            <form action="${ctx}/drawMoney/dealDrawMoneyById">
                                <input type="hidden" name="id" value="<%=drawMoney.getId()%>">
                                <input type="hidden" name="pagesNow" value="<%=pagesNow%>">
                                <button>通过</button>
                            </form>
                            <form action="${ctx}/drawMoney/errorDrawMoney">
                                <input type="hidden" name="id" value="<%=drawMoney.getId()%>">
                                <input type="hidden" name="pagesNow" value="<%=pagesNow%>">
                                <button>不通过</button>
                            </form>
                        </td>
                        <%}else if (drawMoney.getDraw_condition()==1){%>
                            <td style="color: #00B83F">已打款</td>
                        <%}else if (drawMoney.getDraw_condition()==2){%>
        ...
        <div class="humeng" style="margin-left: 50px">
            ...
            <%if (pagesNow!=1){%>
            <div class="humeng">
                <a href="${ctx}/drawMoney/getDrawMoneys?pages=<%=pagesNow-1%>&rows=10" class="pages">上一页</a>
            </div>
            <%}%>
            <div class="humeng">
            ...
            <div class="humeng">
                <a href="${ctx}/drawMoney/getDrawMoneys?pages=<%=pagesNow+1%>&rows=10" class="pages">下一页</a>
            </div>
            <%}%>
            <div class="humeng">
                <form action="${ctx}/drawMoney/getDrawMoneys" id="choosePage">
