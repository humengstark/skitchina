...
<%@ page import="com.skitchina.model.CheckSubmit" %>
<%@ page import="com.skitchina.model.Client" %>
...
<%
    Map clients = (Map) session.getAttribute("clients");
    List<CheckSubmit> checkSubmits = (List<CheckSubmit>) session.getAttribute("checkSubmits");
    int pagesNow = (Integer) session.getAttribute("pagesNow");
    int checkSubmitsNum = (Integer) session.getAttribute("checkSubmitsNum");
    double pagesAll1 = (double) checkSubmitsNum / 10;
    int pagesAll;
    if (pagesAll1 > checkSubmitsNum / 10) {
        pagesAll = checkSubmitsNum / 10 + 1;
    } else {
        pagesAll = checkSubmitsNum / 10;
    }
%>
<!--_meta 作为公共模版分离出去-->
<HTML>
    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            ...
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l"></span> <span class="r">共有数据：<strong><%=checkSubmitsNum%></strong> 条</span> </div>
            <div class="mt-10">
                    ...
                    <tr class="submits">
                        ...
                        <th><%=checkSubmit.getMoney()%></th>
                            ...
                            <%}else if (checkSubmit.getCondition()==1){%>
                                <th style="color: #00B83F">已审核</th>
                        <%}%>
                        <form action="${ctx}/management/getWaybillsByCheckSubmit">
                            <input type="hidden" name="pagesOnCheckSubmits" value="<%=pagesNow%>">
                            <input type="hidden" name="pages" value="1">
                            <input type="hidden" name="id" value="<%=checkSubmit.getId()%>" >
        ...
        <div class="humeng" style="margin-left: 50px">
            ...
            <%if (pagesNow!=1){%>
            <div class="humeng">
                <a href="${ctx}/management/getCheckSubmits?pages=<%=pagesNow-1%>&rows=10" class="pages">上一页</a>
            </div>
            <%}%>
            <div class="humeng">
            ...
            <div class="humeng">
                <a href="${ctx}/management/getCheckSubmits?pages=<%=pagesNow+1%>&rows=10" class="pages">下一页</a>
            </div>
            <%}%>
            <div class="humeng">
                <form action="${ctx}/management/getCheckSubmits" id="choosePage">
