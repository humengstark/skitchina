...
<%@ page import="com.skitchina.model.Notice" %>
...
<%
    ...
    int noticesNum = (Integer) session.getAttribute("noticesNum");
    double pagesAll1 = (double) noticesNum / 10;
    int pagesAll;
    if (pagesAll1 > noticesNum / 10) {
        pagesAll = noticesNum / 10 + 1;
    } else {
        pagesAll = noticesNum / 10;
    }
%>
<!--_meta 作为公共模版分离出去-->
<HTML>
    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            ...
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l"></span> <span class="r">共有数据：<strong><%=noticesNum%></strong> 条</span> </div>
            <div class="mt-10">
                <table class="table table-border table-bordered table-bg table-sort" style="table-layout: fixed">
                    ...
                    <tr class="notices">
                        <td><%=notice.getTime().substring(0,19)%>
                        </td>
                        <td><%=notice.getContent()%>
                        </td>
                        <td>
                            <form action="${ctx}/management/deleteNoticeById">
                    ...
                    <form action="${ctx}/management/addNotice">
                        <input type="text" name="content" style="width: 800px" placeholder="添加公告">
                        <input type="hidden" name="pagesNow" value="<%=pagesNow%>">
                        <button>提交</button>
                    </form>
                </table>
        ...
        <div class="humeng" style="margin-left: 50px">
            ...
            <%if (pagesNow!=1){%>
            <div class="humeng">
                <a href="${ctx}/management/getAllNotices?pages=<%=pagesNow-1%>&rows=10" class="pages">上一页</a>
            </div>
            <%}%>
            <div class="humeng">
            ...
            <div class="humeng">
                <a href="${ctx}/management/getAllNotices?pages=<%=pagesNow+1%>&rows=10" class="pages">下一页</a>
            </div>
            <%}%>
            <div class="humeng">
                <form action="${ctx}/management/getAllNotices" id="choosePage">
