...
<%@ page import="com.skitchina.model.Advertisement" %>
...
<%
    ...
    Map<Integer,Advertisement> advertisements = (Map<Integer,Advertisement>) session.getAttribute("advertisements");
    int pagesNow = (Integer) session.getAttribute("pagesNow");
    int clientsNum = (Integer) session.getAttribute("clientsNum");
    double pagesAll1 = (double) clientsNum / 10;
    int pagesAll;
    if (pagesAll1 > clientsNum / 10) {
        pagesAll = clientsNum / 10 + 1;
    } else {
        pagesAll = clientsNum / 10;
    }
%>
<!--_meta 作为公共模版分离出去-->
<HTML>
    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            ...
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l"></span> <span class="r">共有数据：<strong><%=clientsNum%></strong> 条</span> </div>
            <div class="mt-10">
                <table class="table table-border table-bordered table-bg table-sort" style="table-layout: fixed">
                    ...
                    <tr class="submits">
                        <th><%=client.getCellphone()%></th>
                        <th><%=client.getCompany_name()%></th>
                        <th><%=client.getCompany_address()%></th>
        ...
        <div class="humeng" style="margin-left: 50px">
            ...
            <%if (pagesNow!=1){%>
            <div class="humeng">
                <a href="${ctx}/management/getAllClients?pages=<%=pagesNow-1%>&rows=10" class="pages">上一页</a>
            </div>
            <%}%>
            <div class="humeng">
            ...
            <div class="humeng">
                <a href="${ctx}/management/getAllClients?pages=<%=pagesNow+1%>&rows=10" class="pages">下一页</a>
            </div>
            <%}%>
            <div class="humeng">
                <form action="${ctx}/management/getAllClients" id="choosePage">
